 ...
 * Usage:
 *   node task-cli.js add "Task title" --type feat --priority 1 --status BACKLOG
 *   node task-cli.js update <id> --status IN_PROGRESS --progress 50
 *   node task-cli.js list
 *   node task-cli.js complete <id>
...
const commands = {
  ...
  delete: (args) => {
    ...
    try {
      ...
      if (result.changes > 0) {
        console.log(`🗑️ Task ${taskId} moved to trash`);
      } else {
        console.error(`Task ${taskId} not found`);
      }
    } catch (err) {
